...
inline void PMA_Lock::name(char const about[10]){
#if ENABLE_PMA_LOCK == 1
#ifdef debug
  for (int i = 0; i < 10; i++) {
    info[i] = about[i];
...
inline void PMA_Lock::number(int i){
#if ENABLE_PMA_LOCK == 1
#ifdef debug
  num = i;
#endif
...
inline bool PMA_Lock::check_unlocked() {
#if ENABLE_PMA_LOCK == 1
  return x == 0;
#endif
  return true;
...
inline bool cas( uint64_t *old, uint64_t old_val, uint64_t new_val) {
  return __sync_bool_compare_and_swap(old, old_val, new_val);
}

// exclusive lock
inline bool PMA_Lock::lock(uint64_t task_id, REASONS r) {
#if ENABLE_PMA_LOCK == 1
lock_start:
  assert(task_id > 0);
  assert(r != SAME);
...
inline bool PMA_Lock::try_lock(uint64_t task_id, REASONS r) {
#if ENABLE_PMA_LOCK == 1
  assert(task_id > 0);
  assert(r != SAME);
  assert(num_tries > 0);
...
inline void PMA_Lock::unlock(uint64_t task_id, REASONS r) {
#if ENABLE_PMA_LOCK == 1
  assert(task_id > 0);
#ifdef debug
  assert(num != -1);;
...
inline bool PMA_Lock::i_own_lock(uint64_t task_id) {
#if ENABLE_PMA_LOCK == 1
#ifdef debug
  assert(num != -1);;
#endif
...
inline void PMA_Lock::make_shared(uint64_t task_id) {
#if ENABLE_PMA_LOCK == 1
  assert(task_id > 0);
  ...
  owner = 0;
#endif
  [[maybe_unused]] bool done = cas(&x, 1, 2);
  assert(done);
#endif
}

inline void PMA_Lock::make_exclusive(uint64_t task_id) {
#if ENABLE_PMA_LOCK == 1
  assert(task_id > 0);
#ifdef debug
  assert(num != -1);;
...
inline bool PMA_Lock::lock_shared(uint64_t task_id) {
#if ENABLE_PMA_LOCK == 1
  int tries;
  bool success = false;
  assert(owner == 0);
  ...
  #ifdef debug
    [[maybe_unused]] uint64_t lock_val = x;
  #endif
...
inline void PMA_Lock::unlock_shared(uint64_t task_id) {
#if ENABLE_PMA_LOCK == 1
#ifdef debug
  assert(num != -1);;
#endif
